...
void graph::insertEdge(size_t vertex_1, size_t vertex_2)
{
	...
	adjacency_matrix[vertex_1 * vertices + vertex_2] = true;
	adjacency_matrix[vertex_2 * vertices + vertex_1] = true;
}
...
void graph::depthFirstWalk(size_t vertex_start, vector<bool> &visited)
{
	visited[vertex_start] = true;
	for (size_t v = 0; v < vertices; v++)
		if (adjacency_matrix[vertex_start * vertices + v] == true
				&& visited[v] == false)
			depthFirstWalk(v, visited);
}
